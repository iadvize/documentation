...
The iAdvize Messenger SDK for Android is available through its dedicated demo project on Gith

| Demo project | [https://github.com/iadvize/iadvize-android-sdk](https://github.com/iadvize/iadvize-android-sdk) |
...
After that you can activate the SDK using the `activate` function with your `projectId` (see the [Setting up your iAdvize environment](#💬-setting-up-your-iadvize-environment) section above to get that identifier). You have access to callbacks in order to know if the SDK has been successfully activated. In case of an SDK activation failure the callback will give you the reason of the failure and you may want to retry later:
