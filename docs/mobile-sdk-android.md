...
A demo project is available, a lot of the SDK code snippets used in this documentation can be found there in a real application context: [Demo Project](https://github.com/iadvize/iadvize-android-sdk)
...
After that you can activate the SDK using the `activate` function with your `projectId` (see the [Prerequisites](https://developers.iadvize.com/documentation/mobile-sdk#⚙️-prerequisites) section to get that identifier). You have access to callbacks in order to know if the SDK has been successfully activated. In case of an SDK activation failure the callback will give you the reason of the failure and you may want to retry later:
...
Using a targeting rule UUID (see the [Prerequisites](https://developers.iadvize.com/documentation/mobile-sdk#⚙️-prerequisites) section to get that identifier), you can engage a user by calling:
